 ...
 */
static void publish_callback([[maybe_unused]] void **unused,
...
/**
 * @brief Converts certificate \p xc to a trust anchor in \ta (based on code in
 * BearSSL tools)
 */
static int certificate_to_trust_anchor(br_x509_certificate *xc,
                                       br_x509_trust_anchor *ta);

/**
 * @brief Generates trust anchors for BearSSL from the contents of \p ca_file
 * and stores them in the \p anchoOut array (based on code in BearSSL tools)
 */
static size_t get_trusted_anchors_from_file(const char *ca_file,
                                            br_x509_trust_anchor **anchOut);
/**
 * @brief Generates trust anchors for BearSSL from the string \p ca and stores
 * them in the \p anchOut array (based on code in BearSSL tools)
 *
 * @returns The number of trust anchors generated
 */
static size_t get_trusted_anchors(const unsigned char *ca, size_t ca_len,
                                  br_x509_trust_anchor **anchOut);
...
int main(int argc, const char *argv[]) {
  ...
  /* generate BearSSL trusted anchors - specifically kept out of open_nb_socket
   * since it needs to malloc */

  /*
      Generate BearSSL trusted anchors

      This code converts the certificate into a format that is readable by the
     BearSSL library. Sadly there isn't a way to accomplish this without the use
     of malloc thus I specifically kept this code out of open_nb_socket. The
     author of the bearSSL library offers two options:

      1) Do the conversion of the certificate in your code. There are examples
     of how to do this. The benefit of this is that you can run the same code
     against different servers by providing the appropriate trusted root pem
     file. The function get_trusted_anchors does exactly this.

      2) Use the tool provided with BearSSL to generate the C code that will
     initialize the trusted anchor structures. Essentially it simply generates
     initialized C structures that you can copy into your code. You will not
     need to use malloc but you will lose some flexibility. For information on
     the tool see this page: https://www.bearssl.org/api1.html
  */
  ...
  if (0 !=
      open_nb_socket(&ctx, addr, port, bearssl_iobuf, sizeof(bearssl_iobuf))) {
  ...
  mqtt_init(&client, &ctx, sendbuf, sizeof(sendbuf), recvbuf, sizeof(recvbuf),
            publish_callback);
  ...
  int counter =
      49; /* 49 so it will publish the first time the loop is entered */
  int disconnect = 0;
  ...
  while (!stop) {
    if (++disconnect % 300 == 0) {
      ...
      if (0 != open_nb_socket(&ctx, addr, port, bearssl_iobuf,
                              sizeof(bearssl_iobuf))) {
        fprintf(stderr, "Unable to open socket: %d\n", errno);
        exit_example(EXIT_FAILURE, &ctx);
      }

      if (MQTT_OK != mqtt_connect(&client, "publishing_client", nullptr,
                                  nullptr, 0, nullptr, nullptr, 0, 400)) {
...
static void vblob_append(void *cc, const void *data, size_t len) {
  ...
  if (bv->data_length + len > bv->buffer_length) {
    ...
    bv->buffer = realloc(bv->buffer,
                         bv->buffer_length); // We just have to hope this works
  }
...
static int certificate_to_trust_anchor(br_x509_certificate *xc,
  ...
  case BR_KEYTYPE_RSA:
    ...
    if (nullptr == (ta->pkey.key.rsa.n =
                        (unsigned char *)malloc(ta->pkey.key.rsa.nlen)) ||
        nullptr == (ta->pkey.key.rsa.e =
                        (unsigned char *)malloc(ta->pkey.key.rsa.elen))) {
      free_ta_contents(ta);
      return 0;
    } else {
  ...
  case BR_KEYTYPE_EC:
    ...
    if (nullptr ==
        (ta->pkey.key.ec.q = (unsigned char *)malloc(ta->pkey.key.ec.qlen))) {
      free_ta_contents(ta);
      return 0;
    } else {
    ...
    case BR_PEM_END_OBJ:
      if (inobj) {
        ...
        if (0 == memcmp(po_name, CERTIFICATE, CERTIFICATE_LEN) ||
          ...
          po_name = nullptr;
          if (nullptr ==
              (tas = (br_x509_trust_anchor *)realloc(
                   tas, (cert_count + 1) * sizeof(br_x509_trust_anchor)))) {
